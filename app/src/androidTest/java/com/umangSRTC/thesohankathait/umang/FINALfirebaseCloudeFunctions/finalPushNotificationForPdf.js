...
exports.pushNotificationForPdf = functions.database.ref('{collegeName}/PdfCategory/{schoolName}/{pushId}')
.onCreate( (snapshot,context) => {
console.log('Push notification event triggered for pdf');
...
/* Create a notification and data payload. They contain the notification information, and message to be sent respectively */
    const payload = {
        notification: {
            title: 'There is a new notice available in '+schoolName+" (pdf section)",
...
return admin.messaging().sendToTopic(collegeName+"Tokens", payload, options);
});